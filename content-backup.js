...
(function() {
  ...
  class DataFlattener {
    ...
    // Memoized selective flattening for performance
    static selectiveFlatten(obj, selectedKeys = null, useCache = true) {
      if (useCache) {
        const cacheKey = JSON.stringify({ obj: typeof obj === 'object' ? Object.keys(obj) : obj, selectedKeys });
        if (this.memoCache && this.memoCache.has(cacheKey)) {
      ...
      if (selectedKeys) {
        ...
        result = this.flattenJsonIterative(obj);
      }

      if (useCache && this.memoCache) {
        // Implement LRU cache
  ...
  // Ultra-high-performance table viewer with virtual scrolling and event delegation
  class TableViewer {
    constructor(data) {
      ...
      this.renderCache = new Map(); // LRU cache for rendered rows
      ...
      this.boundSearchHandler = PerformanceUtils.debounce(this.handleSearch.bind(this), 300);
    ...
    extractColumns(data) {
      ...
      // Sample more rows for better column detection, but limit for performance
      const sampleSize = Math.min(data.length, 200);
      data.slice(0, sampleSize).forEach(row => {
        if (row && typeof row === 'object') {
          Object.keys(row).forEach(key => {
            if (key !== '__rowId') { // Exclude internal keys
              columnSet.add(key);
            }
          });
        }
    ...
    renderVirtualized() {
      ...
      const html = `
        <div class="virtual-scroll-container" style="height: ${containerHeight}px; overflow-y: auto;">
          <div class="virtual-scroll-spacer" style="height: ${offsetTop}px;"></div>
          <table class="json2table-table">
            ...
            <tbody>
              ${this.renderRowRange(startIndex, endIndex)}
            </tbody>
          </table>
          <div class="virtual-scroll-spacer" style="height: ${containerHeight - offsetTop - (endIndex - startIndex) * this.rowHeight}px;"></div>
        </div>
      `;
    ...
    calculateVisibleRange() {
      ...
      const endIndex = Math.min(
        ...
        startIndex + Math.ceil(containerHeight / this.rowHeight) + this.bufferSize * 2
      );
    ...
    renderRowRange(startIndex, endIndex) {
      let html = '';
    ...
    renderAllRows() {
      // Use caching for non-virtual rendering
      if (this.renderCache.has('all-rows') && this.filteredData === this.lastFilteredData) {
        return this.renderCache.get('all-rows');
      }
      ...
      // Cache the result
      this.updateCache('all-rows', html);
      this.lastFilteredData = this.filteredData;
      return html;
    }

    renderMainRow(row, rowIndex) {
      const stableRowId = row.__rowId;
      const cacheKey = `row-${stableRowId}`;
    ...
    handleScroll() {
      ...
      if (tbody) {
        tbody.innerHTML = this.renderRowRange(startIndex, endIndex);
      }
    }
    ...
    handleDelegatedClick(event) {
      ...
      // Handle expandable array/object clicks
      ...
      if (target.classList.contains('hover-expandable')) {
        event.preventDefault();
        this.handleImagePreview(target);
        return;
      }
      
      // Handle collapse buttons
      if (target.classList.contains('collapse-array-btn') || target.classList.contains('collapse-object-btn')) {
        event.preventDefault();
        this.handleCollapseClick(target);
        return;
      }
    }

    handleExpansionClick(target) {
      // Implementation for expansion logic
    ...
    attachOptimizedEventListeners() {
      ...
      const searchInput = document.getElementById('json2table-search');
      if (searchInput) {
        searchInput.addEventListener('input', (e) => this.boundSearchHandler(e.target.value));
        this.eventListeners.push({ element: searchInput, event: 'input', handler: this.boundSearchHandler });
      }
    }
